...
namespace Wefabric\Support\Money;
...
class Money extends AbstractMoney
{
     ...
     * @param \Wefabric\Support\Money ...$monies The subsequent monies.
     *
     * @return Money
     *
    ...
    /**
     * @param string $monetaryGroupingSeparatorSymbol
     * @param int $fractionDigits
     * @return string
     */
    public function format(string $monetaryGroupingSeparatorSymbol = '.', int $fractionDigits = 2): string
    {
        $formatter = new \NumberFormatter(config('app.locale'), \NumberFormatter::CURRENCY);
        $formatter->setSymbol(\NumberFormatter::CURRENCY_SYMBOL, $this->getCurrencySymbol());
        $formatter->setSymbol(\NumberFormatter::MONETARY_GROUPING_SEPARATOR_SYMBOL, $monetaryGroupingSeparatorSymbol);
     ...
     */
    public static function formatPrice($price, int $decimals = 2, bool $withCurrencySymbol = true): string
    {
        if(!$price instanceof Money) {
            $price = self::of($price, config('money.currency'), null,  RoundingMode::HALF_UP);
        }
    ...
    {
        return config('money.currency_symbols')[$this->currency->getCurrencyCode()];
    }
